...
public class ClusterOperationExample {
	...
	private static List<HRegionInfo> filterTableRegions(List<HRegionInfo> regions, TableName tableName) {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		HTableDescriptor desc = new HTableDescriptor(tableName).addFamily(coldef1).setValue("Description",
				"Chapter 5 - ClusterOperationExample");
		...
		admin.createTable(desc, regions); // co
											// ClusterOperationExample-01-Create
											// Create a table with seven
											// regions, and one column family.

		BufferedMutator mutator = connection.getBufferedMutator(tableName);
		for (int a = 'A'; a <= 'Z'; a++)
			for (int b = 'A'; b <= 'Z'; b++)
				for (int c = 'A'; c <= 'Z'; c++) {
					String row = Character.toString((char) a) + Character.toString((char) b)
							+ Character.toString((char) c); // co
															// ClusterOperationExample-02-Put
															// Insert many rows
															// starting from
															// "AAA" to "ZZZ".
															// These will be
		...
		System.out.println("Number of regions: " + numRegions); // co
																// ClusterOperationExample-03-List
																// List details
																// about the
																// regions.
		...
		admin.splitRegion(info.getRegionName()); // co
													// ClusterOperationExample-04-Split
													// Split the last region
													// this table has, starting
													// at row key "TUV". Adds a
													// new region starting with
													// key "WEI".
		do {
			list = admin.getTableRegions(tableName);
			Thread.sleep(1 * 1000L);
			System.out.print(".");
		} while (list.size() <= numRegions); // co
												// ClusterOperationExample-05-Wait
												// Loop and check until the
		...
		HRegionLocation location = locator.getRegionLocation(Bytes.toBytes("ZZZ")); // co
																					// ClusterOperationExample-06-Cache
																					// Retrieve
																					// region
																					// infos
																					// cached
		...
		admin.offline(offline.getRegionName()); // co
												// ClusterOperationExample-07-Offline
												// Offline a region and print
		...
		HRegionInfo split = online.get(0); // co
											// ClusterOperationExample-08-Wrong
											// Attempt to split a region with a
											// split key that does not fall into
											// boundaries. Triggers log message.
		System.out.println("Splitting region with wrong key: " + split.getRegionNameAsString());
		admin.splitRegion(split.getRegionName(), Bytes.toBytes("ZZZ")); // triggers
																		// log
																		// message

		System.out.println("Assigning region: " + offline.getRegionNameAsString());
		admin.assign(offline.getRegionName()); // co
												// ClusterOperationExample-09-Reassign
		...
		admin.mergeRegions(m1.getEncodedNameAsBytes(), // co
														// ClusterOperationExample-10-Merge
														// Merge the first two
														// regions. Print out
														// result of operation.
